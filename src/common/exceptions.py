"""Custom exception hierarchy for the   AI application.

All domain-specific exceptions inherit from  AIError so that
...
class SQLExecutionError( AIError):
...
class UnsupportedFileError( AIError):
